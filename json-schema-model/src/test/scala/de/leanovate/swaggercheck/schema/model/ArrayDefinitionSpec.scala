...
class ArrayDefinitionSpec extends WordSpec with MockitoSugar with MustMatchers {
  "ArrayDefinition" should {
    ...
    "succeed if item definition succeeds on all elements" in {
      ...
      when(itemDefinition.validate(schema, path.index(0), item1)).thenReturn(ValidateSuccess)
      when(itemDefinition.validate(schema, path.index(1), item2)).thenReturn(ValidateSuccess)
